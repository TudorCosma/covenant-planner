      ...
      {progress.map((p, i) => {
        ...
        return (
          <div key={g.id} style={{ paddingBottom: i < progress.length - 1 ? 12 : 0, marginBottom: i < progress.length - 1 ? 12 : 0, borderBottom: i < progress.length - 1 ? `1px solid ${COLORS.border}80` : "none" }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: COLORS.text, marginBottom: 4 }}>{g.label}</div>
            {p.sub ? (
              p.sub.map(sub => (
                <div key={sub.id} style={{ marginTop: 6 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: COLORS.text, marginBottom: 2 }}>
                    <span>{sub.label}</span>
                    <span style={{ color: COLORS.text, fontWeight: sub.achieved ? 600 : 400 }}>{sub.summary}{sub.achieved ? " ✓" : ""}</span>
                  </div>
              ...
              <>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: COLORS.text, marginBottom: 2 }}>
                  ...
                  <span style={{ color: COLORS.text, fontWeight: p.achieved ? 600 : 400 }}>{p.summary}{p.achieved ? " ✓" : ""}</span>
                </div>

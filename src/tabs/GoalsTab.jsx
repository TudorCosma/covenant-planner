...
import { Card, Btn, YearSelect } from "../components";
...
function GoalEditor({ goal, onChange, onSave, onCancel, onDelete, personal }) {
  ...
  return (
    <div style={{ background: `${COLORS.border}30`, padding: 14, borderRadius: 8, display: "grid", gap: 10 }}>
      ...
      {goal.kind === "house" && (<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
        <div><label style={labelStyle}>By year</label><YearSelect value={goal.targetYear} onChange={v => set({ targetYear: Number(v) })} personal={personal} small /></div>
      ...
      {goal.kind === "debt" && (
        <div><label style={labelStyle}>Debt-free by year</label><YearSelect value={goal.targetYear} onChange={v => set({ targetYear: Number(v) })} personal={personal} small /></div>
      )}
        ...
        <div style={{ marginTop: 16, display: "grid", gap: 10 }}>
          ...
          {goals.map(g => (
            editing?.goal.id === g.id ? (
              <GoalEditor key={g.id} goal={editing.goal} onChange={ng => setEditing(e => ({ ...e, goal: ng }))} onSave={saveEdit} onCancel={() => setEditing(null)} onDelete={deleteGoal} personal={state.personal} />
            ) : (
          ...
          {editing?.isNew && (
            <GoalEditor goal={editing.goal} onChange={ng => setEditing(e => ({ ...e, goal: ng }))} onSave={saveEdit} onCancel={() => setEditing(null)} onDelete={() => setEditing(null)} personal={state.personal} />
          )}
        </div>
      </Card>
    </div>
